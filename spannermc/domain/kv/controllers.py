...
class KVStoreController(Controller):
    ...
    signature_namespace = {"KeyValueStoreService": KVStoreService, "User": User, "KeyValueStore": KVStore}
    ...
    def list_kv(
        self, kv_service: KVStoreService, filters: list[FilterTypes] = Dependency(skip_validation=True)
    ) -> OffsetPagination[KVStore]:
        """List kv."""
        results, total = kv_service.list_and_count(*filters)
        return kv_service.to_dto(results, total, *filters)
